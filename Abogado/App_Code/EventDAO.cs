...
public class EventDAO
{
    ...
    private static string connectionString = "Data Source =DESKTOP-952N6LL; initial Catalog = ABOGADOS; user = admon; password = 123456; MultipleActiveResultSets = True; App = EntityFramework";
    ...
    public static List<CalendarEvent> getEvents(DateTime start, DateTime end)
    {
        ...
        using (con)
        {
            ...
            while (reader.Read())
            {
                ...
                cevent.lugar = (string)reader["place"];
    ...
    public static void updateEvent(int id, String title, String description, String lugar)
    {
        ...
        cmd.Parameters.AddWithValue("@place", lugar);
    ...
    public static int addEvent(CalendarEvent cevent)
    {
        ...
        cmd.Parameters.AddWithValue("@place", cevent.lugar);
        ...
        using (con)
        {
            ...
            cmd = new SqlCommand("SELECT max(event_id) FROM event where title=@title AND description=@description AND place=@place AND event_start=@event_start AND event_end=@event_end", con);
            cmd.Parameters.AddWithValue("@title", cevent.title);
            cmd.Parameters.AddWithValue("@description", cevent.description);
            cmd.Parameters.AddWithValue("@place", cevent.lugar);
            cmd.Parameters.AddWithValue("@event_start", cevent.start);
